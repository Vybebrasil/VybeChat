...
export default function CloudflareHome() {
  ...
  const sidebar = <aside className={`${mobileSidebarOpen ? "fixed inset-y-0 left-0 z-40 flex w-[292px] shadow-2xl" : "hidden"} cyber-panel flex-col bg-[#0a0b0f]/98 md:relative md:flex md:w-[292px] md:shrink-0`}>
    ...
    <div className="flex-1 overflow-y-auto px-3 py-4">{EXTERNAL_WORKSPACE.map(category => <section key={category.name} className="mb-5"><p className="cyber-label mb-2 px-2 text-orange-300/55">// {category.name}</p>{category.channels.map(channel => {
      const members = voiceRooms[channel.id] ?? [];
      return <div key={channel.id}><button onClick={() => channel.type === "voice" ? joinVoice(channel.id) : selectChannel(channel.id)} className={`mb-0.5 flex h-10 w-full items-center gap-2 border-l-2 px-2.5 text-sm transition-colors ${selectedChannelId === channel.id ? "border-orange-400 bg-orange-400/10 text-orange-100" : "border-transparent text-stone-400 hover:border-orange-300/40 hover:bg-orange-400/5 hover:text-stone-100"}`}>{channel.type === "voice" ? <Volume2 className="size-4" /> : <Hash className="size-4" />}<span className="truncate font-semibold">{channel.name}</span>{channel.type === "voice" && members.length > 0 && <span className="ml-auto font-mono text-[10px] text-orange-300">{members.length}</span>}</button>{members.map(member => <div key={member.socketId} className="ml-7 flex items-center gap-2 py-1 text-[11px] text-stone-500"><span className={`size-1.5 rounded-full ${member.isSpeaking ? "bg-orange-300 shadow-[0_0_10px_rgba(255,145,25,.9)]" : "bg-stone-700"}`} />{member.name}{member.isMuted && <MicOff className="ml-auto size-3 text-rose-300" />}</div>)}</div>;
    })}</section>)}</div>
  ...
  return <main className="cyber-grid min-h-screen p-0 text-foreground md:p-3"><section className="cyber-panel flex min-h-screen overflow-hidden bg-[#0b0c10]/92 md:min-h-[calc(100vh-1.5rem)]">{mobileSidebarOpen && <button onClick={() => setMobileSidebarOpen(false)} className="fixed inset-0 z-30 bg-black/75 md:hidden" aria-label="Fechar navegação" />}{sidebar}
