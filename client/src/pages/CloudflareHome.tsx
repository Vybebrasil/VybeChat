...
import { EXTERNAL_WORKSPACE, findExternalChannel, setExternalWorkspace, type ExternalCategory } from "@/lib/external-workspace";
...
import { dayLabel, formatMessageTime, needsDaySeparator } from "@/lib/message-time";
...
type ExternalMessage = { id: string; channelId: number; userId: string; authorName: string; content: string; createdAt: string; editedAt?: string; parentId?: string | null; reactions?: Record<string, string[]> };
...
export default function CloudflareHome() {
  ...
  const [workspace, setWorkspace] = useState<ExternalCategory[]>(EXTERNAL_WORKSPACE);
  ...
  useEffect(() => {
    ...
    socket.on("message:removed", ({ channelId, messageIds }: { channelId: number; messageIds: string[] }) => {
      const removidos = new Set(messageIds);
      setMessages(current => ({ ...current, [channelId]: (current[channelId] ?? []).filter(item => !removidos.has(item.id)) }));
      setThreadParent(atual => (atual && removidos.has(atual.id) ? null : atual));
    });
    socket.on("call:force-mute", ({ channelId, by }: { channelId: number; by: string }) => {
      if (channelId !== activeCallRef.current) return;
      localStreamRef.current?.getAudioTracks().forEach(track => { track.enabled = false; });
      setMicrophoneOn(false);
      setNotice(`${by} silenciou seu microfone. Ligue de novo quando quiser falar.`);
    });
    socket.on("workspace:list", ({ workspace: recebido }: { workspace: ExternalCategory[] }) => {
  ...
  const editMessage = (messageId: string, content: string) =>
    socketRef.current.emit("message:edit", { channelId: selectedChannelId, messageId, content });

  const deleteMessage = (messageId: string) =>
    socketRef.current.emit("message:delete", { channelId: selectedChannelId, messageId });
  ...
  const muteParticipant = (socketId: string) => {
    if (!activeCallRef.current) return;
    socketRef.current.emit("call:mute", { channelId: activeCallRef.current, socketId });
  };
  ...
  const sidebar = <aside className={`${mobileSidebarOpen ? "fixed inset-y-0 left-0 z-40 flex w-[292px] shadow-2xl" : "hidden"} cyber-panel flex-col bg-[#0a0b0f]/98 md:relative md:flex md:w-[292px] md:shrink-0`}>
    ...
    <div className="flex-1 overflow-y-auto px-3 py-4"><CommandNavigation unread={unread} groups={workspace} selectedChannelId={selectedChannelId} voiceRooms={voiceRooms} onSelectText={selectChannel} onJoinVoice={prepareVoice} />{currentRole === "admin" && <div className="mt-2"><ChannelManager workspace={workspace} onSave={saveWorkspace} /></div>}</div>
