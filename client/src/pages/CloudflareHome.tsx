...
export default function CloudflareHome() {
  ...
  const sidebar = <aside className={`${mobileSidebarOpen ? "fixed inset-y-0 left-0 z-40 flex w-[286px] shadow-2xl" : "hidden"} flex-col border-r border-white/7 bg-[#15101d]/98 md:relative md:flex md:w-[286px] md:shrink-0`}>
    <div className="flex items-start justify-between border-b border-white/7 px-5 py-5"><div><p className="font-mono text-[10px] font-bold uppercase tracking-[.18em] text-violet-300">Vybe OS // Cloud</p><h2 className="mt-1 text-sm font-bold text-white">Central de comunicação</h2></div><button onClick={() => setMobileSidebarOpen(false)} className="grid size-8 place-items-center rounded-lg text-slate-400 hover:bg-white/5 md:hidden" aria-label="Fechar canais"><X className="size-4" /></button></div>
    <div className="flex-1 overflow-y-auto px-3 py-4">{EXTERNAL_WORKSPACE.map(category => <section key={category.name} className="mb-5"><p className="mb-1 px-2 font-mono text-[10px] font-bold tracking-[.12em] text-slate-500">{category.name}</p>{category.channels.map(channel => {
      const members = voiceRooms[channel.id] ?? [];
      return <div key={channel.id}><button onClick={() => channel.type === "voice" ? joinVoice(channel.id) : selectChannel(channel.id)} className={`flex h-9 w-full items-center gap-2 rounded-lg px-2 text-sm ${selectedChannelId === channel.id ? "bg-violet-400/14 text-violet-100" : "text-slate-400 hover:bg-white/5"}`}>{channel.type === "voice" ? <Volume2 className="size-4" /> : <Hash className="size-4" />}<span className="truncate">{channel.name}</span>{channel.type === "voice" && members.length > 0 && <span className="ml-auto text-[10px] text-emerald-300">{members.length}</span>}</button>{members.map(member => <div key={member.socketId} className="ml-7 flex items-center gap-2 py-1 text-[11px] text-slate-400"><span className={`size-2 rounded-full ${member.isSpeaking ? "bg-emerald-300 shadow-[0_0_8px_rgba(52,211,153,.9)]" : "bg-slate-600"}`} />{member.name}{member.isMuted && <MicOff className="ml-auto size-3 text-rose-300" />}</div>)}</div>;
    })}</section>)}</div>
    {activeCallChannelId && <div className="border-t border-violet-300/15 bg-violet-500/8 p-3"><button onClick={() => setCallStageOpen(true)} className="w-full text-left"><p className="text-[11px] font-bold text-emerald-200">Conectado: {findExternalChannel(activeCallChannelId)?.name}</p><p className="mt-1 text-[10px] text-slate-400">Abrir palco da chamada</p></button><div className="mt-2 flex gap-1"><button onClick={toggleMic} className={`grid size-8 place-items-center rounded-lg ${microphoneOn ? "bg-white/8" : "bg-rose-500/20 text-rose-200"}`}>{microphoneOn ? <Mic className="size-4" /> : <MicOff className="size-4" />}</button><button onClick={toggleCamera} className={`grid size-8 place-items-center rounded-lg ${cameraOn ? "bg-white/8" : "bg-rose-500/20 text-rose-200"}`}>{cameraOn ? <Video className="size-4" /> : <VideoOff className="size-4" />}</button><button onClick={shareScreen} className={`grid size-8 place-items-center rounded-lg ${screenStream ? "bg-violet-400/25 text-violet-100" : "bg-white/8"}`}><MonitorUp className="size-4" /></button><button onClick={leaveVoice} className="ml-auto grid size-8 place-items-center rounded-lg bg-rose-500 text-white"><Phone className="size-4 rotate-[135deg]" /></button></div></div>}
    <div className="flex items-center gap-2 border-t border-white/7 p-3"><Avatar className="size-8"><AvatarFallback className="bg-violet-400/15 text-xs text-violet-100">{initials(profile.name)}</AvatarFallback></Avatar><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold text-white">{profile.name}</p><p className="truncate text-[10px] text-emerald-300">Online</p></div><button onClick={() => { leaveVoice(); localStorage.removeItem(PROFILE_KEY); setProfile(null); }}><LogOut className="size-4 text-slate-500" /></button></div>
  </aside>;
