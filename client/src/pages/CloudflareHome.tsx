    ...
    const collect = async () => {
      const entries = await Promise.all(Array.from(peerConnectionsRef.current.entries()).map(async ([peerId, connection]) => {
        try {
          ...
          return [peerId, { sending: false, receiving: false, connection: connection.connectionState }] as const;
        }
      }));
