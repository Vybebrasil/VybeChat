...
import { getAudioActivityLevel, getLoopbackInputWarning, shouldKeepVoiceGateOpen, type VoiceFocusMode } from "@/lib/voice-focus";
...
export default function CloudflareHome() {
  ...
  const [voiceFocusEnabled, setVoiceFocusEnabled] = useState(false);
  const [voiceFocusMode, setVoiceFocusMode] = useState<VoiceFocusMode>("balanced");
  const [voiceFocusLevel, setVoiceFocusLevel] = useState(0);
  const [voiceFocusGateOpen, setVoiceFocusGateOpen] = useState(true);
  const [voiceFocusWarning, setVoiceFocusWarning] = useState<string | null>(null);
  ...
  useEffect(() => {
    const label = audioInputs.find(input => input.deviceId === selectedAudioInput)?.label ?? "";
    setVoiceFocusWarning(getLoopbackInputWarning(label));
  }, [audioInputs, selectedAudioInput]);

  useEffect(() => {
    const stream = localStreamRef.current;
    if (!stream || !activeCallChannelId || !voiceFocusEnabled || pushToTalkEnabled || !microphoneOn) {
      if (stream && !pushToTalkEnabled) stream.getAudioTracks().forEach(track => { track.enabled = microphoneOn; });
      setVoiceFocusGateOpen(Boolean(microphoneOn));
      return;
    }
    const AudioContextClass = window.AudioContext ?? (window as unknown as { webkitAudioContext?: typeof AudioContext }).webkitAudioContext;
    if (!AudioContextClass) {
      setNotice("O foco de voz não é suportado neste navegador. Use fones ou push-to-talk.");
      return;
    }
    ...
    const updateGate = () => {
      ...
      const level = getAudioActivityLevel(samples);
      const now = performance.now();
      if (level >= (voiceFocusMode === "strong" ? 12 : 7)) lastVoiceAt = now;
      const nextGateOpen = shouldKeepVoiceGateOpen(level, lastVoiceAt, now, voiceFocusMode);
      if (nextGateOpen !== gateOpen) {
        gateOpen = nextGateOpen;
        stream.getAudioTracks().forEach(track => { track.enabled = nextGateOpen; });
        setVoiceFocusGateOpen(nextGateOpen);
        socketRef.current.emit("call:audio-state", { channelId: activeCallRef.current, isMuted: !nextGateOpen, isSpeaking: nextGateOpen });
      }
      setVoiceFocusLevel(level);
      frame = requestAnimationFrame(updateGate);
    };
    updateGate();
    return () => {
      ...
      source.disconnect();
      void context.close();
      stream.getAudioTracks().forEach(track => { track.enabled = microphoneOn; });
      setVoiceFocusGateOpen(Boolean(microphoneOn));
    };
  }, [activeCallChannelId, localStream, microphoneOn, pushToTalkEnabled, voiceFocusEnabled, voiceFocusMode]);
  ...
  const toggleVoiceFocus = () => {
    setVoiceFocusEnabled(current => {
      const next = !current;
      if (next) {
        setPushToTalkEnabled(false);
        setNotice("Foco de voz ativado. Sons baixos e pausas serão filtrados; use fones para eliminar áudio de jogo alto.");
      } else {
        setNotice("Foco de voz desativado. O microfone voltará a transmitir continuamente.");
      }
      return next;
    });
  };
  ...
  const sidebar = <aside className={`${mobileSidebarOpen ? "fixed inset-y-0 left-0 z-40 flex w-[292px] shadow-2xl" : "hidden"} cyber-panel flex-col bg-[#0a0b0f]/98 md:relative md:flex md:w-[292px] md:shrink-0`}>
    ...
    {activeCallChannelId && <div className="border-y border-orange-300/15 p-3"><VoiceContextDock roomName={findExternalChannel(activeCallChannelId)?.name ?? "Sala de voz"} participantCount={activeRoomMembers.length} microphoneOn={microphoneOn} cameraOn={cameraOn} screenSharing={Boolean(screenStream)} audioInputs={audioInputs} selectedAudioInput={selectedAudioInput} voiceFocusEnabled={voiceFocusEnabled} voiceFocusMode={voiceFocusMode} voiceFocusLevel={voiceFocusLevel} voiceFocusGateOpen={voiceFocusGateOpen} voiceFocusWarning={voiceFocusWarning} onAudioInputChange={changeAudioInput} onToggleVoiceFocus={toggleVoiceFocus} onVoiceFocusModeChange={setVoiceFocusMode} onToggleMic={toggleMic} onToggleCamera={toggleCamera} onShareScreen={shareScreen} onOpenFocus={() => setCallStageOpen(true)} onLeave={leaveVoice} /></div>}
