...
const props = { roomName: "Sala Geral", participantCount: 2, microphoneOn: true, cameraOn: true, screenSharing: false, audioInputs: [{ deviceId: "mic-1", label: "Microfone USB" }], selectedAudioInput: "", voiceFocusEnabled: false, voiceFocusMode: "balanced" as const, voiceFocusLevel: 0, voiceFocusGateOpen: true, voiceFocusWarning: null, onAudioInputChange: vi.fn(), onToggleVoiceFocus: vi.fn(), onVoiceFocusModeChange: vi.fn(), onToggleMic: vi.fn(), onToggleCamera: vi.fn(), onShareScreen: vi.fn(), onOpenFocus: vi.fn(), onLeave: vi.fn() };
...
describe("VoiceContextDock", () => {
  it("mantém controles, foco e seleção de microfone no contexto da conversa", () => {
    ...
    expect(props.onToggleVoiceFocus).toHaveBeenCalledOnce();
  });
});
