...
import { SCREEN_QUALITY_HINT, SCREEN_QUALITY_LABEL, type ScreenQuality } from "@/lib/screen-share";
...
export function VoiceContextDock({ roomName, participantCount, microphoneOn, cameraOn, screenSharing, audioInputs, selectedAudioInput, onAudioInputChange, gateSensitivity, onGateSensitivityChange, micLevel = 0, screenQuality = "nitida", onScreenQualityChange, onToggleMic, onToggleCamera, onShareScreen, onOpenFocus, onLeave }: VoiceContextDockProps) {
  return <section className="voice-context-dock rounded-2xl border border-orange-300/20 bg-orange-400/[.06] p-3 shadow-[0_14px_30px_rgba(0,0,0,.18)]">
    ...
    {onScreenQualityChange && <div className="mt-3 rounded-xl bg-black/20 px-2.5 py-2">
      <span className="flex items-center gap-1.5 text-[11px] font-semibold text-orange-100"><MonitorUp className="size-3.5 text-orange-300" />Qualidade da tela</span>
      <div className="mt-2 grid grid-cols-2 gap-1.5">
        {(["nitida", "fluida"] as const).map(opcao => <button key={opcao} type="button" onClick={() => onScreenQualityChange(opcao)} aria-pressed={screenQuality === opcao} className={`h-8 rounded-lg text-[11px] font-semibold transition-colors ${screenQuality === opcao ? "bg-orange-400 text-black" : "bg-white/8 text-orange-50 hover:bg-white/12"}`}>{SCREEN_QUALITY_LABEL[opcao]}</button>)}
      </div>
      <span className="mt-1.5 block text-[10px] leading-4 text-stone-500">{SCREEN_QUALITY_HINT[screenQuality]}</span>
    </div>}
    <MicSensitivityMeter className="mt-3 rounded-xl bg-black/20 px-2.5 py-2" level={micLevel} sensitivity={gateSensitivity} onSensitivityChange={onGateSensitivityChange} />
