    ...
    >
      ...
      >
        <strong style={{ display: "block", fontSize: 22 }}>
          VybeChat não pôde abrir agora.
        </strong>
        <p style={{ color: "#a1a1aa", lineHeight: 1.6 }}>
          Tente atualizar a página. Se o problema continuar, a equipe já terá um
          diagnóstico para corrigir.
        </p>
        <button
          type="button"
...
if (
  ...
  void import("./App")
    ...
    .catch(() => root.render(<PublicShellError />));
} else {
  void Promise.all([
    import("./App"),
    ...
    .then(
      ([
        ...
        const redirectToLoginIfUnauthorized = (error: unknown) => {
          if (
            !(error instanceof trpcModule.TRPCClientError) ||
            error.message !== sharedConst.UNAUTHED_ERR_MSG
          )
            return;
        ...
        const trpcClient = trpcBinding.trpc.createClient({
          links: [
            trpcModule.httpBatchLink({
              url: "/api/trpc",
              transformer: superjsonModule.default,
              fetch(input, init) {
