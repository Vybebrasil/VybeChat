...
function setup(
  env: Record<string, string> = { VYBECHAT_WORKSPACE_CODE: WORKSPACE_CODE }
) {
  const storage = new MemoryStorage();
  const admin = new FakeSocket({
    socketId: "admin-socket",
    userId: "gestaovybe@gmail.com",
    name: "Admin",
    status: "online",
    role: "admin",
    callChannelId: null,
...
function packets(socket: FakeSocket) {
...
describe("VybeChatRoom collaboration integration", () => {
  it("persists a thread, broadcasts reactions and returns matching search results", async () => {
    ...
    await room.handleEvent(admin, "message:new", {
      channelId: 1,
      content: "Decisão principal",
    });
    const [root] = (await storage.get("messages:1")) as Array<{
      id: string;
      reactions?: unknown;
    }>;
    await room.handleEvent(member, "message:new", {
      channelId: 1,
      content: "Resposta de apoio",
      parentId: root.id,
    });
    await room.handleEvent(member, "message:reaction", {
      channelId: 1,
      messageId: root.id,
      emoji: "👍",
    });
    await room.handleEvent(member, "message:search", { query: "decisão" });
    const history = (await storage.get("messages:1")) as Array<{
      parentId?: string;
    ...
    expect(
      packets(member).some(
        packet =>
          packet.type === "message:search-results" &&
          packet.payload.results.length === 1
      )
    ).toBe(true);
  });
  ...
  it("enforces pin, read-only and invitation policies by role", async () => {
    ...
    await room.handleEvent(member, "message:pin", {
      channelId: 1,
      messageId: "m1",
    });
    expect(packets(member).at(-1)?.payload.message).toContain(
      "Apenas moderadores"
    );
    await room.handleEvent(admin, "channel:permissions:update", {
      channelId: 1,
      readOnly: true,
      invitePolicy: "admin",
    });
    ...
    await room.handleEvent(member, "call:invite", {
      channelId: 1,
      userId: "gestaovybe@gmail.com",
    });
    expect(
      packets(member).filter(packet => packet.type === "realtime:error")
    ).toHaveLength(3);
  });
  ...
  it("persists a promoted moderator role and permits that moderator to pin a decision", async () => {
    ...
    await room.handleEvent(admin, "team:role:update", {
      userId: "member@vybe.com",
      role: "moderator",
    });
    await room.handleEvent(member, "message:pin", {
  ...
  it("persists an administrator promotion and restores it on a new presence join", async () => {
    ...
    await room.handleEvent(admin, "team:role:update", {
      userId: "lead@vybe.com",
      role: "admin",
    });
    const reconnected = new FakeSocket({
      socketId: "lead-reconnected",
      userId: null,
      name: "Visitante",
      status: "offline",
      role: "member",
    ...
    await room.handleEvent(reconnected, "presence:join", {
      userId: "lead@vybe.com",
      name: "Lead",
      status: "online",
  ...
  it("recusa presence:join quando o codigo de acesso nao confere", async () => {
    ...
    const visitante = new FakeSocket({
      socketId: "visitante",
      userId: null,
      name: "Visitante",
      status: "offline",
    ...
    await room.handleEvent(visitante, "presence:join", {
      userId: "invasor",
      name: "Invasor",
      workspaceCode: "errado",
    });
    expect(visitante.deserializeAttachment().userId).toBeNull();
    expect(packets(visitante).at(-1)).toMatchObject({
      type: "realtime:error",
      payload: { code: "auth" },
    });
  });
  ...
  it("nao entrega historico para conexao que ainda nao autenticou", async () => {
    ...
    expect(
      packets(visitante).some(packet => packet.type === "message:history")
    ).toBe(false);
    expect(packets(visitante).at(-1)).toMatchObject({
      type: "realtime:error",
      payload: { code: "auth" },
  ...
  it("promove a admin pelo slug configurado em VYBECHAT_ADMIN_SLUGS", async () => {
    const { room, sockets } = setup({
      VYBECHAT_WORKSPACE_CODE: WORKSPACE_CODE,
      VYBECHAT_ADMIN_SLUGS: "paulo, mizinho",
    });
    const chefe = new FakeSocket({
      socketId: "chefe",
      userId: null,
      name: "Visitante",
      status: "offline",
    ...
    sockets.push(chefe);
    await room.handleEvent(chefe, "presence:join", {
      userId: "paulo-m1abc-x9y2",
      name: "Paulo",
      workspaceCode: WORKSPACE_CODE,
    });
  ...
  it("nao promove quem apenas comeca com um trecho do slug", async () => {
    const { room, sockets } = setup({
      VYBECHAT_WORKSPACE_CODE: WORKSPACE_CODE,
      VYBECHAT_ADMIN_SLUGS: "paulo",
    });
    const outro = new FakeSocket({
      socketId: "outro",
      userId: null,
      name: "Visitante",
      status: "offline",
    ...
    sockets.push(outro);
    await room.handleEvent(outro, "presence:join", {
      userId: "paulozinho-m1abc-x9y2",
      name: "Paulozinho",
      workspaceCode: WORKSPACE_CODE,
    });
  ...
  it("informa o proprio socketId no presence:join", async () => {
    const { room, sockets } = setup();
    const novo = new FakeSocket({
      socketId: "socket-do-paulo",
      userId: null,
      name: "Visitante",
      status: "offline",
      role: "member",
    ...
    sockets.push(novo);
    await room.handleEvent(novo, "presence:join", {
      userId: "paulo-1",
      name: "Paulo",
      workspaceCode: WORKSPACE_CODE,
    });
    // O cliente usa esse id para decidir quem cede numa colisao de ofertas.
    expect(packets(novo)[0]).toEqual({
      type: "session:ready",
      payload: { socketId: "socket-do-paulo" },
    });
  });
  ...
  it("eventos de chamada so vao para quem esta na mesma sala de voz", async () => {
    ...
    const entrando = new FakeSocket({
      socketId: "entrando",
      userId: "novo@vybe.com",
      name: "Novo",
      status: "online",
      role: "member",
      callChannelId: null,
    ...
    const naSala5 = packets(admin).filter(
      packet => packet.type === "call:peer-joined"
    );
    const naSala6 = packets(member).filter(
      packet => packet.type === "call:peer-joined"
    );
  ...
  it("mantém presença entre motores sem misturar sinalização P2P com participantes do SFU", async () => {
    const { room, sockets, admin, member } = setup();
    admin.attachment.callChannelId = 5;
    admin.attachment.callEngine = "legacy";
    member.attachment.callChannelId = 5;
    member.attachment.callEngine = "realtimekit";
    const entrando = new FakeSocket({
      socketId: "novo-sfu",
      userId: "novo@vybe.com",
      name: "Novo",
    ...
    await room.handleEvent(entrando, "call:join", {
      channelId: 5,
      mediaEngine: "realtimekit",
    });

    expect(entrando.deserializeAttachment().callEngine).toBe("realtimekit");
    expect(
      packets(admin).some(packet => packet.type === "call:peer-joined")
    ).toBe(false);
    expect(packets(member).some(packet => packet.type === "voice:rooms")).toBe(
      true
    );
    expect(packets(entrando).some(packet => packet.type === "call:peers")).toBe(
      false
    );
  });
  ...
  it("o autor edita a própria mensagem e o texto novo chega a todos", async () => {
    ...
    await room.handleEvent(admin, "message:new", {
      channelId: 1,
      content: "versão errada",
    });
    const [msg] = (await storage.get("messages:1")) as Array<{ id: string }>;
    await room.handleEvent(admin, "message:edit", {
      channelId: 1,
      messageId: msg.id,
      content: "versão corrigida",
    });
    const historico = (await storage.get("messages:1")) as Array<{
      content: string;
      editedAt?: string;
    }>;
  ...
  it("ninguém reescreve a fala de outra pessoa, nem moderador", async () => {
    ...
    await room.handleEvent(admin, "message:edit", {
      channelId: 1,
      messageId: msg.id,
      content: "texto trocado",
    });
    expect(
      ((await storage.get("messages:1")) as Array<{ content: string }>)[0]
        .content
    ).toBe("minha mensagem");
  ...
  it("apagar leva junto as respostas da thread", async () => {
    ...
    await room.handleEvent(admin, "message:delete", {
      channelId: 1,
      messageId: raiz.id,
    });
  ...
  it("quem não é autor nem moderador não apaga", async () => {
    ...
    const [msg] = (await storage.get("messages:1")) as Array<{ id: string }>;
    await room.handleEvent(member, "message:delete", {
      channelId: 1,
      messageId: msg.id,
    });
  ...
  it("apagar uma mensagem fixada tira ela dos fixados", async () => {
    ...
    const [msg] = (await storage.get("messages:1")) as Array<{ id: string }>;
    await room.handleEvent(admin, "message:pin", {
      channelId: 1,
      messageId: msg.id,
    });
    await room.handleEvent(admin, "message:delete", {
  ...
  it("moderador silencia alguem para toda a sala", async () => {
    ...
    admin.attachment.callChannelId = 5;
    member.attachment.callChannelId = 5;
    await room.handleEvent(admin, "call:mute", {
      channelId: 5,
      socketId: "member-socket",
    });
    ...
    expect(
      packets(member).find(p => p.type === "call:force-mute")
    ).toMatchObject({ payload: { by: "Admin" } });
  ...
  it("quem nao e moderador nao silencia ninguem", async () => {
    ...
    admin.attachment.callChannelId = 5;
    member.attachment.callChannelId = 5;
    await room.handleEvent(member, "call:mute", {
      channelId: 5,
      socketId: "admin-socket",
    });
  ...
  it("a busca encontra mensagem em canal criado depois", async () => {
    ...
    await room.handleEvent(admin, "workspace:update", {
      workspace: [
        {
          name: "OPERACAO",
          channels: [{ id: 1, name: "geral", type: "text" }],
        },
        {
          name: "NOVA",
          channels: [{ id: 90, name: "clientes", type: "text" }],
        },
      ],
    });
    await room.handleEvent(admin, "message:new", {
      channelId: 90,
      content: "briefing do cliente novo",
    });
    ...
    const resultado = packets(admin)
      .filter(p => p.type === "message:search-results")
      .pop();
  ...
  it("persists direct messages, emits them to the recipient and clears the local unread counter", async () => {
    const { room, storage, admin, member } = setup();
    await room.handleEvent(admin, "direct:new", {
      toUserId: "member@vybe.com",
      toName: "Member",
      content: "Pode revisar a entrega?",
    });
    await room.handleEvent(member, "direct:list", {});
    await room.handleEvent(member, "direct:history", {
      peerUserId: "gestaovybe@gmail.com",
    });
    await room.handleEvent(member, "direct:read", {
      peerUserId: "gestaovybe@gmail.com",
    });
    const index = (await storage.get("direct:index:member@vybe.com")) as Array<{
      unreadCount: number;
    }>;
    ...
    expect(
      packets(member).some(
        packet =>
          packet.type === "direct:new" &&
          packet.payload.message.content === "Pode revisar a entrega?"
      )
    ).toBe(true);
    expect(
      packets(member).some(
        packet =>
          packet.type === "direct:history" &&
          packet.payload.messages.length === 1
      )
    ).toBe(true);
  });
  ...
  it("sincroniza o pedido de fala de um participante da sala", async () => {
    ...
    await room.handleEvent(member, "call:hand-raise", {
      channelId: 5,
      active: true,
    });
    const rooms = packets(member)
      .filter(packet => packet.type === "voice:rooms")
      .at(-1)?.payload;
  ...
  it("persiste decisões e permite que o responsável conclua a própria ação", async () => {
    ...
    await room.handleEvent(member, "decision:create", {
      title: "Aprovar roteiro",
      ownerName: "Member",
      dueDate: "2026-08-22",
    });
    const [decision] = (await storage.get("team:decisions")) as Array<{
      id: string;
      status: string;
      createdBy: string;
    }>;
    await room.handleEvent(member, "decision:update", {
      id: decision.id,
      status: "done",
    });
    const [updated] = (await storage.get("team:decisions")) as Array<{
      status: string;
      createdBy: string;
    }>;
    ...
    expect(
      packets(member).some(
        packet =>
          packet.type === "decision:list" &&
          packet.payload.decisions.length === 1
      )
    ).toBe(true);
  });

  it("persiste a fila musical antes de sincronizar somente a sala de voz", async () => {
    const { room, storage, admin, member, sockets } = setup();
    admin.attachment.callChannelId = 5;
    member.attachment.callChannelId = 5;
    const outraSala = new FakeSocket({
      socketId: "outra-sala",
      userId: "outra@vybe.com",
      name: "Outra",
      status: "online",
      role: "member",
      callChannelId: 6,
    });
    sockets.push(outraSala);

    await room.handleEvent(admin, "music:enqueue", {
      channelId: 5,
      source: { kind: "video", videoId: "dQw4w9WgXcQ" },
    });

    const stored = (await storage.get("music:channel:5")) as {
      queue: Array<{ videoId: string }>;
      playing: boolean;
      djUserId: string;
    };
    expect(stored.queue[0]?.videoId).toBe("dQw4w9WgXcQ");
    expect(stored.playing).toBe(true);
    expect(stored.djUserId).toBe("gestaovybe@gmail.com");
    expect(packets(member).some(packet => packet.type === "music:state")).toBe(
      true
    );
    expect(
      packets(outraSala).some(packet => packet.type === "music:state")
    ).toBe(false);
  });

  it("deixa integrantes adicionar à fila, mas reserva a reprodução ao DJ", async () => {
    const { room, storage, admin, member } = setup();
    admin.attachment.callChannelId = 5;
    member.attachment.callChannelId = 5;
    await room.handleEvent(admin, "music:enqueue", {
      channelId: 5,
      source: { kind: "playlist", playlistId: "PL1234567890" },
    });
    await room.handleEvent(member, "music:enqueue", {
      channelId: 5,
      source: { kind: "video", videoId: "M7lc1UVf-VE" },
    });
    await room.handleEvent(member, "music:control", {
      channelId: 5,
      action: "pause",
      positionSeconds: 12,
    });
  ...
  it("restaura a música atual para quem entra depois na chamada", async () => {
    const { room, admin, member } = setup();
    admin.attachment.callChannelId = 5;
    await room.handleEvent(admin, "music:enqueue", {
      channelId: 5,
    ...
    await room.handleEvent(member, "call:join", {
      channelId: 5,
      mediaEngine: "realtimekit",
    });

    const restored = packets(member).find(
      packet => packet.type === "music:state"
    );
    expect(restored?.payload.state.queue[0]?.videoId).toBe("dQw4w9WgXcQ");
    expect(restored?.payload.state.playing).toBe(true);
  });

  it("recusa fontes que não são ids válidos do YouTube", async () => {
    const { room, storage, admin } = setup();
    admin.attachment.callChannelId = 5;
    await room.handleEvent(admin, "music:enqueue", {
      channelId: 5,
      source: { kind: "video", videoId: "javascript:" },
    });
    expect(await storage.get("music:channel:5")).toBeUndefined();
    expect(packets(admin).at(-1)).toMatchObject({ type: "realtime:error" });
  });
});
